...
3. **Daemon** (`internal/daemon/`) — HTTP server with embedded HTML dashboard. Core components:
   ...
   - `serviceworker.go` + `sw.js` — Browser-resident offline fallback for `local.vibe`. The dashboard registers `/sw.js`; when the daemon/redirect is unreachable the worker serves a cached "redirect is down — run `vibe doctor --fix`" page (races the real fetch against a 1s timeout, probes the direct HTTP port to distinguish "redirect down" from "daemon down", auto-retries). Solves the chicken-and-egg where the inline dashboard banner can't load precisely when the redirect is down.
...
`sudo vibe setup` installs: dnsmasq, `/etc/resolver/vibe`, pf LaunchDaemon (port 80→7999 and 443→7443), TLS certificates (local CA + leaf, trusted in macOS Keychain), enables TLS in config, user LaunchAgent (daemon at login), and the global agent skill (`~/.claude/skills/local-vibe`, unless `--no-skill`). De-escalates brew/launchctl ops via `SUDO_USER` (the skill install writes to the real user's home and chowns it back). `vibe uninstall` reverses every step.

**pf redirect resilience (`cmd/pf_darwin.go`):** The `com.vibe.pf` LaunchDaemon runs the hidden `vibe pf-apply` at boot (`RunAtLoad`) **and on every network change** (`WatchPaths` on resolv.conf — fires on VPN up/down), so a VPN/firewall flushing pf doesn't silently break `*.vibe` HTTPS until reboot. `pf-apply` is idempotent (no-op when the redirect is already present) and **merges** vibe's rdr into the live ruleset, bucketing captured rules by pf section so the reload is well-ordered — it coexists with another pf user (e.g. ExpressVPN's `xvpn` anchor) instead of replacing the whole ruleset. A logged minimal-ruleset fallback covers an unparseable merge. Known gap: if the VPN's kill-switch *blocks/route-to's* loopback (not just flushes pf), `pf-apply` restores the rules but the redirect can still be intercepted — that's a VPN "allow LAN access" setting, not a vibe bug. Event-driven, no polling.

**`vibe doctor` (`cmd/doctor.go`):** Diagnoses each layer of the request path — daemon reachable (via the client: unix socket → TCP), HTTP/TLS listeners, DNS resolves `*.vibe`, the privileged-port redirect (a loopback dial to :80/:443, the layer a VPN breaks), and cert presence. `vibe doctor --fix` repairs the redirect by re-running `pf-apply` via sudo (per-OS hook: `doctor_darwin.go` / `doctor_other.go`). The redirect probe (shared `internal/netprobe`) needs no root; it's also surfaced as a `vibe list`/`vibe status` warning and the service-worker fallback page. `vibe dev` restarts the daemon deterministically with `launchctl kickstart -k` (avoids launchd's ~10s respawn throttle) and confirms readiness via the client transport, not a raw TCP-7999 dial.
