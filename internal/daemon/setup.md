...
Vibe also injects `AUTH_URL` and `NEXTAUTH_URL` (both
`http://localhost:<callback port>`) into the app's environment. Frameworks that
otherwise derive their base URL from the listening port — Auth.js v5 with no
`AUTH_URL` set — would send the app's auto-assigned port as the `redirect_uri`
and the provider would reject it as a mismatch. Injecting real environment
variables is deliberate: Node loaders (including Next.js) don't overwrite
variables already present in the environment, so this keeps working when a
`.env` file is regenerated by something like `vercel env pull`.
...
**Worktrees inherit the parent's bridge automatically.** All of an app's
worktrees share one provider registration: when a worktree app redirects the
browser to its OAuth provider, vibe tags the request's `state` with the
worktree's route name (injecting a synthetic state for PKCE-only flows), and
the bridge routes the provider's callback back to the worktree's own
`.{{TLD}}` origin — where its session cookies live. No provider-console
changes per worktree. The app must build its redirect URI from fixed config
(e.g. `AUTH_URL=http://localhost:<bridge port>`), not from its listening
port; only server-side authorize redirects can be tagged (a provider URL
assembled in client-side JS never passes through the proxy).

Vibe also copies the parent checkout's untracked `.env*` files into a
worktree that lacks them when it spawns the worktree's process (existing
files are never overwritten) — `git worktree add` doesn't carry gitignored
env files, and auth breaks without them.
...
which case you get a picker listing them (starting main stays one click
away). The picker also **discovers unregistered worktrees on disk** (via
`git worktree list` on the app's directory): a worktree nobody ran
`vibe start` in shows up with a "click to launch" affordance that registers
it with the app's own `cmd` and starts it. Notes: the worktree's
...
The vibe API (`/_api/...`) is unauthenticated — the CLI reaches it over a unix
socket, and the dashboard reaches it from `local.{{TLD}}`. Because it is also
reachable from any browser tab, vibe rejects **state-changing** API calls
(anything but a read) that come from another site, using the `Origin` and
`Sec-Fetch-Site` headers a browser sets and page scripts cannot forge. Without
this, a page on any website could POST a new managed route — and a managed
route's `cmd` runs as you.

What this means in practice:

- The CLI, `curl`, and scripts are unaffected; they send no `Origin`.
- The dashboard and a route's own vibe-served pages (start page, reconnecting
  page) work normally.
- Bookmark routes are **not** trusted origins. They reverse-proxy a
  third-party site onto a `.{{TLD}}` host, and that site is never allowed to
  drive the API.
...
or call the API from outside the browser.
